...
境界を守る4つのSPMパッケージ + 殻に徹する app ターゲット。
...
- **FApp** (F.xcodeproj): SwiftUI + NSViewRepresentable + CAMetalLayer直描画。上記4つを組む

SPMレベルの依存は DecodeKit→DNGKit のみ。CacheKit / XMPKit は独立。
...
バージョンは `MARKETING_VERSION` を上げてからタグを打つ。CIは push/PR で4パッケージの
